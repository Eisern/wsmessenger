...
Wrapped room keys use X25519 ECDH + HKDF-SHA256 (info: `"ws-e2ee-wrap-v2"`, salt: ephemeral pubkey) → AES-256-GCM. Binary format (prefix byte `0x02`): `[version(1)] [ephemeral_pubkey(32)] [IV(12)] [ciphertext+tag]`, base64-encoded.

A wrap can also say **who made it** (prefix `0x03`, behind `KEY_WRAP_SIG_WRITE_ENABLED`, off until readers ship): `[0x03] [ephemeral_pubkey(32)] [IV(12)] [sig(64)] [signer_len(1)] [signer] [ciphertext+tag]`. The signature covers `"ws-keywrap-v1"`, the scope (`room`/`dm`) and its id, the signer, the recipient, the key id and the ciphertext — so a genuine wrap cannot be re-filed as another room's key or another member's copy. The v2 wrap is anonymous (ephemeral ECDH) and the rows are written by the server, which is what lets a dishonest server hand you a key it made itself; `inspectWrappedKey` + the caller's check is what answers that. Unsigned wraps are still accepted — they are every key handed out before this existed — and a signed one that fails verification is **refused**, not merely flagged. Messages use AES-256-GCM with power-of-2 padding buckets (starting at 64 bytes, 5-byte header) and a `kid` field for key versioning/archival.
