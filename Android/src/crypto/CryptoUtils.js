...
const CryptoUtils = {
  ...
  async encryptRoomKeyForUser(peerPublicKeyBase64, roomKeyBase64, signOpts = null) {
    ...
    // Signed variant: the sharer's name and signature go INSIDE the blob, so a
    // recipient can ask who gave them this key rather than taking the server's
    // word for it. See _keyWrapSigMessage for what the signature binds.
    if (signOpts && signOpts.seed && signOpts.signer) {
      const sigMsg = this._keyWrapSigMessage({
        ...
        ephemPub: ephPubRaw, iv, ciphertext: ctBytes,
      });
      const sig = new Uint8Array(this.base64ToArrayBuffer(this.ed25519Sign(signOpts.seed, sigMsg)));
      ...
      out.set(ephPubRaw, at); at += this._WRAP_EPHEM_LEN;
  ...
  async decryptRoomKeyForUser(privateKey, encryptedBase64) {
    ...
    // v3 says the same thing as v2 with the sharer's name and signature in the
    // middle; unwrapping does not care who signed, only where the ciphertext
    // starts. Who signed is what inspectWrappedKey answers, and whether they
    // were entitled to is a question for the caller that knows the room.
    const signed = blob.length > 0 && blob[0] === this._WRAP_VERSION_SIGNED;
